...
cur_enr_df %>% 
  group_by(Student) %>% 
  filter(Presence == "Present" | Presence == "Late" | Presence == "Left Early") %>% 
  count(name = "in_class", .drop = FALSE) -> att_table
...
att_table %>% mutate(absent = class_days_n - in_class) %>% arrange(desc(in_class)) -> att_table

# att_table %>% filter(in_class == max(in_class))
...
`r paste(poor$Student, collapse = ";  ")`

```{r table, echo=FALSE}
kable(list(
  att_table[1:ceiling(nrow(att_table)/2),],
  att_table[(ceiling(nrow(att_table)/2) + 1):nrow(att_table),]
  ),
